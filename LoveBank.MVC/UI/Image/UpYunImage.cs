...
{
    public class UpYunImage:IHtmlString
    {
        ...
        public UpYunImage(string name, string value,UpYun upYun) {

            var policy = upYun.GetPolicy();
            var Sign = upYun.GetSignature();
            ...
            _editor = "KindEditor.ready(function(K){var editor=K.editor({allowFileManager:true,uploadJson:'"+upYun.UploadUrl+"',extraFileUploadParams:{policy:'" + policy + "',signature:'" + Sign + "'}});K('#imgUplaod_" + name + "').click(function(){editor.loadPlugin('image',function(){editor.plugin.imageDialog({imageUrl:K('#" + name + "').val(),clickFn:function(url,title,width,height,border,align){K('#" + name + "').val(url);$(\"#img_icon_" + name + "\").attr('src',url);$(\"#img_del_icon_" + name + "\").css('display','block');editor.hideDialog()}})})})});";
